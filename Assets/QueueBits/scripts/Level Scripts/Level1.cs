...
namespace QueueBits
{
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnNextLevel.transform.position.z);
        ...
        {
			// Star System
			if (GameManager.saveData.starSystem[1] == 0)
			{
		...
		IEnumerator Won()
		{
			...
			if(gameOver == true)
			{
				...
				*/
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[1] + 1 <= 3)
					{
						GameManager.saveData.starSystem[1] = GameManager.saveData.starSystem[1] + 1;
						GameManager.Save();
					}
				}
