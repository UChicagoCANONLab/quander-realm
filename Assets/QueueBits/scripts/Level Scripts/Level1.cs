...
namespace QueueBits
{
	public class Level1 : MonoBehaviour 
	{
		...
		*/
		...
		void Start () 
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[1])
            {
				dialoguePhase = true;
                Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level1");
                DialogueManager.playDialogue[1] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnNextLevel.transform.position.z);
        ...
        {
			// Star System
			if (StarSystem.levelStarCount[1] == 0)
			{
		...
		IEnumerator Won()
		{
			...
			if(gameOver == true)
			{
				...
				*/
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[1] + 1 <= 3)
					{
						StarSystem.levelStarCount[1] = StarSystem.levelStarCount[1] + 1;
					}
				}
				...
				// Reward System
				if (RewardManager.rewardManager[1])
                {
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 1);
				}
			}
			else 
			{
