...
namespace QueueBits
{
	public class Level9 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[9])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level9");
				DialogueManager.playDialogue[9] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[9] + 1 <= 3)
					{
						StarSystem.levelStarCount[9] = StarSystem.levelStarCount[9] + 1;
					}
				}
				// StarSystem
		...
		{
			// Star System
			if (StarSystem.levelStarCount[9] == 0)
			{
