...
namespace QueueBits
{
	public class Level9 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[9])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level9");
				GameManager.saveData.dialogueSystem[9] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 9";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[9] + 1 <= 3)
					{
						GameManager.saveData.starSystem[9] = GameManager.saveData.starSystem[9] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[9])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 9);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[9] == 0)
			{
