...
namespace QueueBits
{
	public class Level4 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[4])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level4");
				DialogueManager.playDialogue[4] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[4] + 1 <= 3)
					{
						StarSystem.levelStarCount[4] = StarSystem.levelStarCount[4] + 1;
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[4])
				{
		...
		{
			// Star System
			if (StarSystem.levelStarCount[4] == 0)
			{
