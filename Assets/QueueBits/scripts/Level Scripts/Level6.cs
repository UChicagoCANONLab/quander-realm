...
namespace QueueBits
{
	public class Level6 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[6])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level6");
				DialogueManager.playDialogue[6] = false;
			...
			levelText.GetComponent<TextMesh>().text = "Level 6";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[6] + 1 <= 3)
					{
						StarSystem.levelStarCount[6] = StarSystem.levelStarCount[6] + 1;
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[6])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 6);
				}
			}
			else
			{
		...
		{
			// Star System
			if (StarSystem.levelStarCount[6] == 0)
			{
