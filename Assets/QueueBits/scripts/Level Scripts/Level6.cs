...
namespace QueueBits
{
	public class Level6 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[6])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level6");
				GameManager.saveData.dialogueSystem[6] = false;
				GameManager.Save();
...
//			tutorial_button.SetActive(true);
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[6] == 0)
			{
