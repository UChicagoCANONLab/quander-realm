...
namespace QueueBits
{
	public class Level15 : MonoBehaviour
	{
		...
		Dictionary<int, List<(Piece, int, int, int)>> prefilledBoardList = new Dictionary<int, List<(Piece, int, int, int)>>
		{
			...
			{4, new List<(Piece piece, int col, int row, int prob)> //8.9 flipped
				{
					(Piece.Red, 3, 5, 100),
				(Piece.Blue, 4, 5, 75),
				(Piece.Red, 4, 4, 75),
			...
			{5, new List<(Piece piece, int col, int row, int prob)> //8.10 flipped
				{
					(Piece.Red, 3, 5, 100),
				(Piece.Blue, 3, 4, 100),
				(Piece.Red, 3, 3, 75),
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[15])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level15");
				GameManager.saveData.dialogueSystem[15] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 15";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[15] + 1 <= 3)
					{
						GameManager.saveData.starSystem[15] = GameManager.saveData.starSystem[15] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[15])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 15);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[15] == 0)
			{
