...
namespace QueueBits
{
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnNextLevel.transform.position.z);
		...
		{
			// Star System
			if (StarSystem.levelStarCount[2] == 0)
			{
		...
		IEnumerator Won()
		{
			...
			if(gameOver == true)
			{
				...
				*/
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[2] + 1 <= 3)
					{
						StarSystem.levelStarCount[2] = StarSystem.levelStarCount[2] + 1;
					}
				}
				...
				if (StarSystem.levelStarCount[2] < 3)
				{
