...
namespace QueueBits
{
	public class Level14 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[14])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level14");
				DialogueManager.playDialogue[14] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[14] + 1 <= 3)
					{
						StarSystem.levelStarCount[14] = StarSystem.levelStarCount[14] + 1;
					}
				}
				// StarSystem
				...
				if (StarSystem.levelStarCount[14] < 3)
				{
		...
		{
			// Star System
			if (StarSystem.levelStarCount[14] == 0)
			{
