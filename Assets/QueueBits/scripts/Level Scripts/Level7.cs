...
namespace QueueBits
{
	public class Level7 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[7])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level7");
				DialogueManager.playDialogue[7] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[7] + 1 <= 3)
					{
						StarSystem.levelStarCount[7] = StarSystem.levelStarCount[7] + 1;
					}
				}
				// StarSystem
		...
		{
			// Star System
			if (StarSystem.levelStarCount[7] == 0)
			{
