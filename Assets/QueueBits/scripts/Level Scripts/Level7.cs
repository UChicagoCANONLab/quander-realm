...
namespace QueueBits
{
	public class Level7 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[7])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level7");
				GameManager.saveData.dialogueSystem[7] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 7";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[7] + 1 <= 3)
					{
						GameManager.saveData.starSystem[7] = GameManager.saveData.starSystem[7] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[7])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 7);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[7] == 0)
			{
