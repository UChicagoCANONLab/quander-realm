...
namespace QueueBits
{
	public class Level10 : MonoBehaviour
	{
		...
		bool currentlyRevealing = false; //Shivani: renamed choosingReveals
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[10])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level10");
				GameManager.saveData.dialogueSystem[10] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 10";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[10] + 1 <= 3)
					{
						GameManager.saveData.starSystem[10] = GameManager.saveData.starSystem[10] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[10])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 10);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[10] == 0)
			{
