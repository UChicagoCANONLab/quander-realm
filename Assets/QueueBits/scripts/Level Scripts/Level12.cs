...
namespace QueueBits
{
	public class Level12 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[12])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level12");
				GameManager.saveData.dialogueSystem[12] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 12";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[12] + 1 <= 3)
					{
						GameManager.saveData.starSystem[12] = GameManager.saveData.starSystem[12] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[12])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 12);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[12] == 0)
			{
