...
{6, new List<(Piece piece, int col, int row)> //5.8
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[5])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level5");
				GameManager.saveData.dialogueSystem[5] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 5";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[5] + 1 <= 3)
					{
						GameManager.saveData.starSystem[5] = GameManager.saveData.starSystem[5] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[5])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 5);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[5] == 0)
			{
