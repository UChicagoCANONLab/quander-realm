...
{6, new List<(Piece piece, int col, int row)> //5.8
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[5])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level5");
				DialogueManager.playDialogue[5] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[5] + 1 <= 3)
					{
						StarSystem.levelStarCount[5] = StarSystem.levelStarCount[5] + 1;
					}
				}
				// StarSystem
		...
		{
			// Star System
			if (StarSystem.levelStarCount[5] == 0)
			{
