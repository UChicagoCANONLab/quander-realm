...
namespace QueueBits
{
	public class Level3 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (DialogueManager.playDialogue[3])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level3");
				DialogueManager.playDialogue[3] = false;
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (StarSystem.levelStarCount[3] + 1 <= 3)
					{
						StarSystem.levelStarCount[3] = StarSystem.levelStarCount[3] + 1;
					}
				}
				// StarSystem
				...
				if (StarSystem.levelStarCount[3] < 3)
				{
		...
		{
			// Star System
			if (StarSystem.levelStarCount[3] == 0)
			{
