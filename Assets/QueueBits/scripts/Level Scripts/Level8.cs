...
namespace QueueBits
{
	public class Level8 : MonoBehaviour
	{
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[8])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level8");
				GameManager.saveData.dialogueSystem[8] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 8";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[8] + 1 <= 3)
					{
						GameManager.saveData.starSystem[8] = GameManager.saveData.starSystem[8] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (RewardManager.rewardManager[8])
				{
					Wrapper.Events.CollectAndDisplayReward?.Invoke(Wrapper.Game.QueueBits, 8);
				}
			}
			else
			{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[8] == 0)
			{
