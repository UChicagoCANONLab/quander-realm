...
namespace QueueBits
{
	public class Level13 : MonoBehaviour
	{
		...
		{
			{0, new List<(Piece piece, int col, int row, int prob)> //7.1 flipped
				{
					(Piece.Red, 3, 5, 75),
				(Piece.Blue, 3, 4, 50),
				(Piece.Red, 4, 5, 50),
			...
			{1, new List<(Piece piece, int col, int row, int prob)> //7.2 flipped
				{
					(Piece.Red, 3, 5, 50),
				(Piece.Blue, 3, 4, 75),
				(Piece.Red, 2, 5, 100),
		...
		void Start()
		{
			...
			// dialogue
			if (GameManager.saveData.dialogueSystem[13])
			{
				dialoguePhase = true;
				Wrapper.Events.StartDialogueSequence?.Invoke("QB_Level13");
				GameManager.saveData.dialogueSystem[13] = false;
				GameManager.Save();
			...
			levelText.GetComponent<TextMesh>().text = "Level 13";
	...
	(numColumns - 1) / 2.0f, -((numRows - 1) / 2.0f) - 1, btnPlayAgain.transform.position.z);
		...
		IEnumerator Won()
		{
			...
			if (gameOver == true)
			{
				...
				{
					starUpdated = true;
					if (GameManager.saveData.starSystem[13] + 1 <= 3)
					{
						GameManager.saveData.starSystem[13] = GameManager.saveData.starSystem[13] + 1;
						GameManager.Save();
					}
				}
				// StarSystem
				...
				// Reward System
				if (GameManager.rewardSystem[13])
				{
		...
		{
			// Star System
			if (GameManager.saveData.starSystem[13] == 0)
			{
